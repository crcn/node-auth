...
exports.model = function(options) {
	...
	var Account = new Schema({
		...
		password: { type: String, required: true, set: hashPass, select: false },
	...
	if(!(usernameField = options.usernameField)) {
	...
	Account.statics.login = function(credentials, callback) {
		...
		if(credentials.token) {
			...
			var q = {};
			q[usernameField] = credentials[usernameField] || credentials.email;
			q.password = hashPass(String(credentials.password));
